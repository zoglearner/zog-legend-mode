...
function sexTally() {
  ...
  return '名单 ' + state.roster.length + ' / ' + MAX +
    ' <span>·</span> 牡 <b class="' + cls(m) + '">' + m + '</b> / 5' +
    ' <span>·</span> 牝 <b class="' + cls(f) + '">' + f + '</b> / 5' +
    ' <span>·</span> 还需 牡 ' + Math.max(0, 5 - m) + ' · 牝 ' + Math.max(0, 5 - f);
}
...
function pickBody() {
  return '<div class="pkbar">' +
    ...
    '整批换池（本轮剩余 ×' + state.refreshes + '）</button>' +
    '</div>' +
    '<div class="candgrid" id="candGrid">' + state.batch.map(candCard).join('') + '</div>' +
    '<div class="sidenote">点卡片只<b>选中</b>（不立即入名单），再点一次可取消；' +
    '点底部「确认加入名单」才计入（防误划）。换池是<b>整批</b>换掉 10 条，每轮 3 次（换轮重置）、不消耗轮次；' +
    '已入名单的马不再出现在后续候选里。' +
    '卡上<b>不直接标注档位</b>，须由生産牧場名自行判断。</div>';
}
...
function renderPick() {
  ...
  $('pkPick').innerHTML = c
    ? '已选中 <b>' + designation() + '</b> · ' + (c.sex === 'M' ? '牡' : '牝') + ' 2 歳 · 父 ' +
      esc(sireOf(c)) + ' · 生産 ' + esc(breederOf(c)) +
      ' <span class="muted">（再点同一张卡可取消）</span>'
    : '点卡片<b>选中</b>一匹 —— 选中后这里会显示它，再点底部「确认加入名单」才计入；' +
      '确认后弹窗收起、回到名单，名单里随即多出这一行。';
...
function viewDraft() {
  ...
  const head = full
    ...
    : '<div class="shead"><h2>选马 · 第 ' + round + ' 轮</h2>' +
      '<p>点名单里那个<b>空位</b>抽出本轮 <b>10 条候选</b>：点卡片选中 → 弹窗底部「确认加入名单」，' +
      '关窗即回到这份名单。<b>确认后自动换上一批全新候选，不消耗换池次数</b>；' +
      '每轮可<b>整批换池 ' + REFRESH_PER_ROUND + ' 次</b>（换轮重置，本轮剩余 ×' + state.refreshes + '）。' +
      '<button class="rules-link" data-rules="1" style="margin-left:8px">规则说明</button></p></div>';
